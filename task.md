...
- [x] 1013 Dart tests + 48 rules tests + 12 goldens; **94.8%** line coverage against an 85% floor
- [x] `tool/check_coverage.dart` — zero-dependency gate, justified exclusions
      → [spec 0007](specs/0007-quality-gates/spec.md)
- [x] CI: lockfile → format → codegen freshness → analyze → test → coverage → build matrix
- [x] `pubspec.lock` must be what the pinned Flutter resolves — catches Dependabot
      committing a lock generated by a newer SDK than `FLUTTER_VERSION`, which was
      silent before: pub just downgraded whatever the SDK caps and stayed green
      → [0007-R16](specs/0007-quality-gates/spec.md)
