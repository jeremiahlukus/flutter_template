...
- **Realtime listeners on Firestore.** Sync is explicit — triggered by the user
  or by a reconnect ([0011](../0011-connectivity/spec.md)). Swapping in `snapshots()` is
  a small change if an app wants it.
- **Attachment sync.** See [0006](../0006-file-storage/spec.md).
...
- ~~Should `sync()` run automatically on connectivity restore?~~ **Resolved by
  [0011](../0011-connectivity/spec.md):** `ReconnectSyncCoordinator` pushes the queue on
  an offline→online transition.
- Nothing retries on a *failed* sync other than the next reconnect or a manual
  tap. Backoff is deliberately out of scope; see [0011](../0011-connectivity/spec.md).
